...
public class CustomConfig extends YamlConfiguration {
     ...
     */
    public CustomConfig(String name, Plugin plugin) throws IOException, InvalidConfigurationException {
        this.plugin = plugin;

        configFile = new File(plugin.getDataFolder(), name.endsWith(".yml")
                ? name
                : name + ".yml"
        );
    ...
    public void save() {
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "There was an error while saving the "
                    + configFile.getName() + " configuration!", e);
        }
    }
    ...
    public void reload() {
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "There was an error while creating the "
                    + configFile.getName() + " configuration file!", e);
        } catch (InvalidConfigurationException e) {
            plugin.getLogger().log(Level.SEVERE, "The configuration \""
    ...
    public String getString(final String path, final String defaultValue, final boolean translateColors) {
        val value = getString(path, defaultValue);

        return value != null && translateColors
